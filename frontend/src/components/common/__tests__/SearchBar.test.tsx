...
import { SearchServiceImpl } from '../../../infrastructure/search/SearchService';
...
describe('SearchBar', () => {
  ...
  describe('基础渲染', () => {
    it('应该正确渲染搜索框', () => {
      render(<SearchBar<any> onSearchResult={() => {}} />);
    ...
    it('应该使用自定义占位符', () => {
      ...
      render(<SearchBar<any> onSearchResult={() => {}} placeholder={placeholder} />);
  ...
  describe('搜索功能', () => {
    it('应该在输入时触发搜索', async () => {
      const mockResult = { items: [{ id: 1, name: 'test' }] };
      mockSearch.mockResolvedValueOnce(mockResult);

      const onSearchResult = vi.fn();
      render(<SearchBar<any> onSearchResult={onSearchResult} />);
  ...
  describe('错误处理', () => {
    it('应该处理搜索错误', async () => {
      ...
      const onError = vi.fn();
      render(<SearchBar<any> onSearchResult={() => {}} onError={onError} />);
