...
describe('AlertRuleForm', () => {
  ...
  describe('表单验证', () => {
    it('validates required fields', async () => {
      render(<AlertRuleForm onSubmit={mockOnSubmit} onCancel={() => {}} />);

      // 触发表单提交
      fireEvent.submit(screen.getByRole('form'));

      // 等待错误消息出现
      const nameError = await screen.findByText('规则名称不能为空');
    ...
    it('validates threshold values', async () => {
      ...
      // 修改期望：查找包含正确的错误提示文本
      const thresholdError = await screen.findByText('阈值不能为负数');
      expect(thresholdError).toBeInTheDocument();
    });

    test('validates email format', async () => {
      render(<AlertRuleForm onSubmit={vi.fn()} onCancel={vi.fn()} />);
