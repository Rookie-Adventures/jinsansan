...
const defaultConfig: ErrorConfig = {
  retry: {
    ...
    retryDelay: 1000,
    maxDelay: 5000,
    retryableErrors: [HttpErrorType.NETWORK, HttpErrorType.TIMEOUT],
  },
  notification: {
...
const getConfig = (): ErrorConfig => {
  try {
    ...
    // 过滤掉无效的配置值
    const filteredConfig = Object.entries(envConfig).reduce<PartialConfig>(
      (acc, [key, section]) => {
        if (section) {
          const filteredSection = Object.entries(section).reduce<Record<string, unknown>>(
